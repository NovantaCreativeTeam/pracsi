...
export default class ElanPlugin extends WaveSurfer.BasePlugin {
    /**
     * Elan plugin definition factory
     *
     * @param  {ElanPluginParams} params parameters use to initialise the plugin
     * @return {ElanPlugin} an instance of the plugin
     */
    static create(params) {
        return new ElanPlugin(params);
    }
    ...
    load(url) {
        this.loadXML(url, xml => {
            if (!xml) {
                console.error('ELAN Plugin: No XML data received');
                return;
            }
            try {
                ...
                console.error('ELAN Plugin: Error parsing ELAN data', e);
            }
        });
    }
    ...
    loadXML(url, callback) {
        ...
        xhr.addEventListener('load', e => {
            ...
            if (xml.getElementsByTagName('parsererror').length > 0) {
                console.error('ELAN Plugin: Error parsing XML from ' + url);
                console.error('Parser error:', xml.getElementsByTagName('parsererror')[0].textContent);
                callback && callback(null);
            } else {
        ...
        xhr.addEventListener('error', () => {
            console.error('ELAN Plugin: Network error while loading ' + url);
            callback && callback(null);
        });
    }
    ...
    parseElan(xml) {
        ...
        if (!header) {
            throw new Error('ELAN document missing <HEADER>');
        }
                ...
                node => {
                    const annot = {
                        ...
                        value: node
                            ...
                            ? node.querySelector('ANNOTATION_VALUE').textContent.trim()
                            : ''
                    };
    ...
    render() {
        ...
        tiers.forEach((tier, index) => {
            tier.annotations.forEach(annot => {
                const alignable =
                    annot.type == this.Types.ALIGNABLE_ANNOTATION
                        ? annot
                        : annot.reference;

                if (alignable) {
                    if (!(alignable.id in backRefs)) {
                        backRefs[alignable.id] = {};
                    }
                    backRefs[alignable.id][index] = annot;
                    indeces[index] = true;
                }
            });
        });
        ...
        this.renderedAlignable.forEach(alignable => {
            ...
            row.className = 'wavesurfer-alignable-row';
    ...
    bindClick() {
        this._onClick = e => {
            ...
            if (null != ref) {
                ...
                if (annot) {
                    this.emit('select', annot.start, annot.end);
                }
            }
        };
    ...
    getRenderedAnnotations(time) {
        return this.renderedAlignable.filter(annotation => {
            return annotation.start <= time && annotation.end >= time;
        });
    }
    ...
    highlight(time) {
        const annotations = this.getRenderedAnnotations(time);
        const activeClass = 'wavesurfer-active-row';

        // Remove active class from all rows
        const rows = this.table.querySelectorAll('.wavesurfer-alignable-row');
        rows.forEach(row => row.classList.remove(activeClass));

        if (annotations.length > 0) {
            annotations.forEach((annotation, index) => {
                const node = this.getAnnotationNode(annotation);
                if (node) {
                    node.classList.add(activeClass);

                    // Optional: scroll into view (only for the first active annotation to avoid jumpy scrolling)
                    if (this.params.scrollToActive && index === 0) {
